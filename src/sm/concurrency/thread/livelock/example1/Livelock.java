package sm.concurrency.thread.livelock.example1;

import java.util.concurrent.locks.Lock;
import java.util.concurrent.locks.ReentrantLock;

/** livelock - two threads aren't blocked, but they can't progress. They will keep withdrawing, trying to do the transer,
 *  failing, and refunding the money. Then they'll look back and try to do the transfer again, until we manually 
    ...
    public boolean withdraw(double amount) {
        if (lock.tryLock()) {
        	try {
        		...
        		lock.unlock(); // To avoid livelock situation
        	}
        }
        return false;
    }
